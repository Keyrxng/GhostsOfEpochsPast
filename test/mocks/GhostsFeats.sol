...
import {DataTypes, IProfileNFT} from "../../src/interfaces/IProfileNFT.sol";
import {IGhosts, IGhostsData} from "../../src/interfaces/IGhosts.sol";
import {Ownable} from  "@openzeppelin/access/Ownable.sol";
import {IERC721} from "@openzeppelin/token/ERC721/IERC721.sol";
import {ICyberNFTBase} from "../../src/interfaces/ICyberNFTBase.sol";
...
contract GhostsFeats  {
    ...
    function createAchievements(
        ...
        uint essID = featsMasterList.length;
    ...
    function createContent(
        ...
        bool allowComments
            ...
            IGhostsData.MiniPost memory newPost;
                newPost.creator = ghostsAddr;
                newPost.creatorCCID = ghostsCCID;
    ...
    // function getAllPosts() public view returns (IGhostsData.MiniPost[] memory){
    //     return posts;
    // }
    ...
    function createGhostsCC(string memory handle, string[] memory hashes, address op) internal returns(uint) {        
        ...
        IGhostsData.UserFeats storage u = getUser[ghostsAddr];
        string[] memory hashes = new string[](2);
        hashes[0] = 'lol';
        hashes[1] = 'sdasd';
        uint256[] memory empty;
        uint256 ccID = IGhosts(ghostsAddr).createUser(handle, hashes);
        if(u.userAddress == address(0)){
            IGhostsData.UserFeats memory newUser = IGhostsData.UserFeats({
