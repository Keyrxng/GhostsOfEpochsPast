...
contract Ghosts is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    function ccGetSubNFTAddr(uint profileId) public view returns (address) {
        return ProfileNFT.getSubNFT(profileId);
    }
    ...
    function ccRegEssence(
        ...
        DataType.RegisterEssenceParams memory params;
        params.profileId = profileId;
        params.name = name;
        params.symbol = symbol;
        params.essenceURI = essenceURI;
        params.essenceMw = essenceMw;
        params.transferable = transferable;
        params.deployAtReg = deployAtReg;
    ...
    function ccSetMetadata(uint profileId, string calldata metadata) internal returns (bool) {
        return ProfileNFT.setMetadata(profileId, metadata); 
    }

    function ccSetSubData(uint profileId, string calldata uri, address mw, bytes calldata mwData) internal returns (bool) {
        return ProfileNFT.setSubscribeData(profileId, uri, mw, mwData);
    }

    function ccSetEssData(uint profileId, uint essId, string calldata uri, address mw, bytes calldata mwData) internal returns (bool) {
        return ProfileNFT.setCollectibleData(profileId, essId, uri, mw, mwData);
    }
